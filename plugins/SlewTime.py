from datetime import timedelta
from ginga.qtw import Widgets
from qtpy import QtCore, QtGui

import PlBase


class SlewTime(PlBase.Plugin):
    """ Show the remaining slew time. """
    aliases = ['STATS.SLEWING_TIME', 'STATS.SLEWING_STATUS']

    def build_gui(self, container):
        self.root = container
        self.root.set_margins(0, 0, 0, 0)
        self.root.set_spacing(0)

        self.lbl = Widgets.Label()
        w = self.lbl.get_widget()
        w.setAlignment(QtCore.Qt.AlignLeft | QtCore.Qt.AlignVCenter)
        w.setStyleSheet("QLabel {color :green ; background-color:white }")
        fontfamily = "Sans"
        self.smfont = QtGui.QFont(fontfamily, 12, QtGui.QFont.Normal)
        w.setFont(self.smfont)

        self.root.add_widget(self.lbl, stretch=1)

    def start(self):
        self.controller.register_select(str(self), self.update, self.aliases)

    def update(self, statusDict):
        self.logger.info('status=%s' % str(statusDict))
        if statusDict['STATS.SLEWING_STATUS'] == 'NO':
            self.lbl.set_text("Slew Time: (Not Slewing)")
            return

        time_sec = statusDict['STATS.SLEWING_TIME']
        if isinstance(time_sec, float):
            mn, sec = divmod(time_sec, 60)
            hr, mn = divmod(mn, 60)
            hr, mn, sec = int(hr), int(mn), int(sec)
            self.lbl.set_text(f"Slew Time: {hr:02d}:{mn:02d}:{sec:02d}")
        else:
            self.lbl.set_text(f"Slew Time: ERROR")


    def __str__(self):
        return 'slewtime'
